function [kernel_c1,kernel_c2,weight_c2,weight_full_1,weight_full_2,weight_arr2num]=CNN_upweight2(Error,train_data,state_c1,state_s1,state_c2,state_s2_temp1,state_s2,...
...
for m=1:layer_c2_num
    for n=1:layer_f2_num
        ...
        for k=1:layer_f1_num
            delta_weight_arr2num_temp(:,:,m)=2*Error(1,n)*weight_full_1(m,k)*weight_full_2(k,n)*state_s2_temp1(:,:,m);
...
for m=1:layer_c2_num
    for n=1:layer_f2_num
        ...
        for k=1:layer_f1_num
            delta_state_s2_temp1(:,:,m)=2*Error(1,n)*weight_full_1(m,k)*weight_full_2(k,n)*weight_arr2num(:,:,m);
...
for n=1:layer_f2_num
    for m=1:layer_c2_num
        ...
        for kk=1:layer_f1_num
             delta_state_s2_temp1(:,:,m)=2*Error(1,n)*weight_full_1(m,kk)*weight_full_2(kk,n)*weight_arr2num(:,:,m);
             count_delta_state_s2_temp1=count_delta_state_s2_temp1+delta_state_s2_temp1(:,:,m);
        end
        delta_state_c2=kron(count_delta_state_s2_temp1,ones(2,2)/4);
